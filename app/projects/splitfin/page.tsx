...
export default function SplitFinPage() {
  const data = caseStudyData.splitfin
  ...
  return (
    <div className="min-h-screen bg-[#020203]">
      ...
      <section className="mx-auto max-w-[1480px] px-4 py-20 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-[0.35fr_0.65fr]">
          {/* Left */}
          <div className="flex flex-col gap-6">
            <span className="text-primary/80 text-xs font-semibold tracking-[0.2em] uppercase">
          ...
          <div className="flex flex-col gap-4">
            {data.productStory.workflow.map((item, i) => (
              <div key={i} className="flex items-start gap-4">
                <div
                  className={`flex size-10 shrink-0 items-center justify-center rounded-full border ${item.accent === 'violet'
                ...
                {i < data.productStory.workflow.length - 1 && (
                  <ChevronDown className="text-muted-foreground/20 mt-6 size-4" />
                )}
              </div>
            ))}
          </div>
